/**
 * SpotsTab コンポーネント（観光スポットタブ）
 *
 * スポット名・メモを持つ観光スポットを追加・削除できる。
 *
 * Props:
 *   items    - スポット項目の配列 [{ id, name, memo }]
...
export default function SpotsTab({ items, onChange }) {
  /**
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-sky-50 rounded-2xl border border-sky-100 p-4 space-y-2">
        <p className="text-sky-600 text-xs font-bold mb-3">✏️ 観光スポットを追加</p>

        <div>
          <label className="text-gray-500 text-xs mb-1 block">スポット名 *</label>
          <input
            type="text"
            name="name"
            value={newItem.name}
      ...
      {items.length === 0 ? (
        <div className="text-center py-10 text-gray-400">
          ...
          <p className="text-sm">観光スポットを追加しましょう</p>
        </div>
      ) : (
